...
async function backupCachePkgs({ usbDev } = {}) {
  ...
  // Detect full pfSense version (e.g. 2.8.1-RELEASE → 2.8.1)
  const fullVer = await runCmd(client, `cat /etc/version | cut -d- -f1`);
  if (!fullVer) {
    console.error(`${c.red}Could not detect pfSense version${c.reset}`);
    process.exit(1);
  }
  console.log(`\npfSense version: ${fullVer}`);
  ...
  // --- RESTAPI package ---
  // Query GitHub API for the latest release and find the matching asset name
  console.log(`\n${c.bold}1. Caching RESTAPI package...${c.reset}`);
  const releasesJson = await runCmd(client,
    `fetch -qo - 'https://api.github.com/repos/pfrest/pfSense-pkg-RESTAPI/releases?per_page=5' 2>/dev/null`
  );

  // Extract latest tag and matching asset for this pfSense version
  const tagMatch   = releasesJson.match(/"tag_name":"([^"]+)"/);
  const assetMatch = releasesJson.match(new RegExp(`"name":"(pfSense-${fullVer.replace('.', '\\.').replace('.', '\\.')}-pkg-RESTAPI\\.pkg)"`));
  const latestTag  = tagMatch  ? tagMatch[1]  : null;
  const assetName  = assetMatch ? assetMatch[1] : `pfSense-${fullVer}-pkg-RESTAPI.pkg`;

  if (!latestTag) {
    console.error(`  ${c.red}Could not determine latest release tag from GitHub API${c.reset}`);
  } else {
    const destPath = `${pkgsDir}/${assetName}`;
    const url      = `https://github.com/pfrest/pfSense-pkg-RESTAPI/releases/download/${latestTag}/${assetName}`;
    console.log(`  Release: ${latestTag}, asset: ${assetName}`);
    const fetchOut = await runCmd(client, `fetch -o '${destPath}' '${url}' 2>&1 && echo OK || echo FAIL`);
    if (fetchOut.includes('OK') && !fetchOut.includes('Not Found')) {
      const size = await runCmd(client, `ls -lh '${destPath}' | awk '{print $5}'`);
      console.log(`  ${c.green}Cached (${size})${c.reset}`);
    } else {
      console.error(`  ${c.red}Fetch failed: ${fetchOut.split('\n').slice(-2).join(' ')}${c.reset}`);
    }
  }
