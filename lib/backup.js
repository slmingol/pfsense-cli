...
async function backupInstall({ usbDev, keepLast, schedule } = {}) {
  ...
  // Step 2: write script to USB line-by-line (command_prompt has a 1024-char limit)
  console.log(`\n${c.bold}2. Writing backup script to ${scriptDest}...${c.reset}`);
  await runCmd(client, `rm -f '${scriptDest}'`);
  ...
  for (const line of lines) {
    ...
    const part = `printf '%s\\n' '${esc}' >> '${scriptDest}'`;
    if (batchLen > 0 && batchLen + 2 + part.length > MAX_CMD) {
      await flushBatch();
    }
    batch.push(part);
  ...
  console.log(`  ${c.green}Written (${lines.length} lines)${c.reset}`);
