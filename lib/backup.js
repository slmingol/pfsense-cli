...
| unifi/ | UniFi controller backup rsync (written by pfSense cron) |
...
Step 2 — Reinstall 3rd-party packages (not included in config restore)
  ...
  Required:
    pfSense-pkg-RESTAPI      (3rd party — see install-api.sh below)
    pfSense-pkg-haproxy
    pfSense-pkg-acme
...
Step 7 — Remove stale UniFi cron entries
  ...
  UniFi no longer runs on pfSense. Remove these two stale cron entries
  via System > Cron (requires pfSense-pkg-Cron) or edit config.xml:
    rsync .../UniFi/data/backup /mnt/usb_backup/unifi/
    rsync .../UniFi/data/backup slm@ghost-files:.../unifi/
