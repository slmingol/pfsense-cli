...
async function showIfaceOptics(client, iface) {
  ...
  const hasDDM = ddm.plugged || ddm.vendor || (ddm.rx_dbm !== null && ddm.rx_dbm !== undefined) || ddm.temperature;
  ...
  if (hasDDM) {
    ...
    if (ddm.tx_dbm !== null && ddm.tx_dbm !== undefined) console.log(`    TX pwr : ${colorDbm(ddm.tx_dbm)}`);
    if (ddm.rx_dbm !== null && ddm.rx_dbm !== undefined) console.log(`    RX pwr : ${colorDbm(ddm.rx_dbm)}`);
