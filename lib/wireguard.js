...
const { createOrUpdateAlias, deleteAlias } = require('./firewall');
const fs = require('fs');
...
async function listTunnels() {
  ...
  try {
    ...
    throw new Error(`Failed to list tunnels: ${error.response?.data?.message || error.message}`, { cause: error });
  }
}
...
async function applyProtonVPN({
  ...
  if (killSwitchAlias) {
    console.log(`  Kill-switch   : alias ${c.cyan}${killSwitchAlias}${c.reset}${killSwitchHosts.length ? ` ← ${killSwitchHosts.join(', ')}` : ''}`);
  } else if (killSwitchHosts.length > 0) {
  ...
  if (!killSwitchAlias && killSwitchHosts.length === 0) {
    console.log(`${c.blue}[7/8]${c.reset} ${c.gray}No kill-switch hosts or alias specified -- skipping LAN routing rules${c.reset}`);
  } else if (killSwitchAlias) {
    // Alias mode: one pass + one block rule referencing the alias; add IPs to alias if provided.
    console.log(`${c.blue}[7/8]${c.reset} Kill-switch alias ${c.cyan}${killSwitchAlias}${c.reset} → single rule pair`);

    if (killSwitchHosts.length > 0) {
      await createOrUpdateAlias({
        name:        killSwitchAlias,
        type:        'host',
        description: `Kill-switch hosts routed via ${tunnelDescr}`,
    ...
    const ruleDescr = `pf-protonvpn-ks-${killSwitchAlias}`;
    ...
    // Fallback block so kill-switch holds when VPN gateway is offline.
    const blockDescr   = `pf-protonvpn-ks-fallback-${killSwitchAlias}`;
    ...
    if (existingBlock) {
      ...
      console.log(`  ${c.yellow}!${c.reset} Drag fallback block immediately after the pass rule above`);
    }
  } else {
    // Per-IP mode (legacy): one pass + one block rule per host CIDR.
  ...
  if (killSwitchAlias) {
    console.log(`   rule "pf-protonvpn-ks-${killSwitchAlias}"  gateway: ${c.cyan}${gwGroupName}${c.reset}`);
  } else {
...
async function teardownProtonVPN({ tunnelDescr, ifaceName, gatewayName, killSwitchAlias = null }) {
  const client = getPfSenseClient();
  const gwName = gatewayName || `${ifaceName}_GW`.toUpperCase();
  ...
  // Remove kill-switch alias if one was managed by this CLI
  if (killSwitchAlias) {
    try {
      await deleteAlias({ name: killSwitchAlias });
    } catch (e) {
      console.warn(`  ${c.yellow}warn${c.reset} Could not remove alias ${killSwitchAlias}: ${e.message}`);
    }
  }
