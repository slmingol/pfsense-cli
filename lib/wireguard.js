...
async function listTunnels() {
  ...
  try {
    const response = await client.get('/api/v2/vpn/wireguard/tunnels');
    ...
    const peersResp = await client.get('/api/v2/vpn/wireguard/peers');
...
async function applyProtonVPN({
  ...
  {
    const resp = await client.get('/api/v2/vpn/wireguard/tunnels');
  ...
  {
    const resp = await client.get('/api/v2/vpn/wireguard/peers');
  ...
  let internalIfaceId;
  {
    const resp = await client.get('/api/v2/interfaces');
  ...
  {
    const resp = await client.get('/api/v2/routing/gateways');
  ...
  {
    const resp = await client.get('/api/v2/firewall/nat/outbound/mappings');
  ...
  {
    const resp = await client.get('/api/v2/firewall/rules');
  ...
  if (killSwitchHosts.length === 0) {
    ...
    const rulesResp = await client.get('/api/v2/firewall/rules');
...
async function teardownProtonVPN({ tunnelDescr, ifaceName, gatewayName }) {
  ...
  try {
    const natResp = await client.get('/api/v2/firewall/nat/outbound/mappings');
  ...
  // Remove gateway
  try {
    const gwResp = await client.get('/api/v2/routing/gateways');
  ...
  try {
    const ifacesResp = await client.get('/api/v2/interfaces');
  ...
  try {
    const tunnelsResp = await client.get('/api/v2/vpn/wireguard/tunnels');
