...
async function addRule({
  ...
  console.log(`  ${c.green}✓${c.reset} Created ${typeColor}${type.toUpperCase()}${c.reset} rule${description ? ': ' + description : ''}${id !== null && id !== undefined ? c.gray + ' (id=' + id + ')' + c.reset : ''}`);
  console.log(`  ${c.gray}✓ Firewall applied${c.reset}`);
}
...
async function deleteRule({ id, description }) {
  ...
  let ruleId = (id !== null && id !== undefined) ? Number(id) : null;
...
async function listPortForwards({ filter } = {}) {
  const client = getPfSenseClient();
  const resp = await client.get('/api/v2/firewall/nat/port_forwards', { params: { limit: 0 } });
  let rules = resp.data.data || [];
  ...
  for (const r of rules) {
    const disabled = r.disabled ? ` ${c.gray}[disabled]${c.reset}` : '';
    const proto    = r.protocol || '*';
    const dst      = r.destination_port ? `${r.destination}:${r.destination_port}` : r.destination;
    const tgt      = r.local_port && r.local_port !== r.destination_port
      ? `${r.target}:${r.local_port}`
      : `${r.target}:${r.destination_port}`;

    console.log(
      `\n  ${c.bold}${String(r.id).padEnd(4)}${c.reset}` +
      ` ${c.cyan}${r.interface.padEnd(6)}${c.reset}` +
...
async function addPortForward({
  ...
  const payload = {
    ...
    descr:            description,
    natreflection:    null,
    associated_rule_id: addRule ? 'pass' : '',
  };

  const resp = await client.post('/api/v2/firewall/nat/port_forward', payload);
  await client.post('/api/v2/firewall/apply');

  const id = resp.data.data?.id;
  const lp = localPort || destPort;
  console.log(`  ${c.green}✓${c.reset} Created port forward: ${c.cyan}${destination}:${destPort}${c.reset}  →  ${c.green}${target}:${lp}${c.reset}${id !== null && id !== undefined ? c.gray + ' (id=' + id + ')' + c.reset : ''}`);
  if (addRule) console.log(`  ${c.gray}✓ Associated firewall pass rule created${c.reset}`);
  console.log(`  ${c.gray}✓ Firewall applied${c.reset}`);
}
...
async function deletePortForward({ id, description }) {
  ...
  if (ruleId === null) {
    ...
    if (rules.length > 1)  throw new Error(`Multiple port forwards match "${description}" — use --id instead`);
    ruleId = rules[0].id;
  }

  await client.delete('/api/v2/firewall/nat/port_forward', { data: { id: ruleId } });
  await client.post('/api/v2/firewall/apply');

  console.log(`  ${c.green}✓${c.reset} Deleted port forward id=${ruleId}`);
  console.log(`  ${c.gray}✓ Firewall applied${c.reset}`);
}
