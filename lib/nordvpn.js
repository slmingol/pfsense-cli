...
async function rotateNordVPNWG({
  ...
  // 0. Check gateway status — skip rotation if tunnel is down
  const gwResp = await client.get('/api/v2/status/gateways', { params: { limit: 0 } });
  const gw     = (gwResp.data.data || []).find(g => g.name === gatewayName);
  if (!gw) {
    console.log(`  ${c.yellow}Gateway ${gatewayName} not found — skipping rotation${c.reset}\n`);
    return;
  }
  if (gw.status !== 'online') {
    console.log(`  ${c.yellow}Gateway ${gatewayName} is ${gw.status} — skipping rotation${c.reset}\n`);
    return;
  }
  ...
  // 5. Update watchdog peer conf on pfSense so the watchdog uses the new endpoint
