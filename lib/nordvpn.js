...
async function rotateNordVPNWG({
  ...
  console.log(`\n${c.green}${c.bold}Rotation complete.${c.reset}`);

  console.log(`\n${c.bold}${c.yellow}Next: update the watchdog peer conf on pfSense${c.reset}`);
  console.log(`  ${c.cyan}cat > /var/db/protonvpn-wg-peer.conf << 'PEER_EOF'`);
  console.log(`  PEER_PK=${newPubkey}`);
  console.log(`  ENDPOINT=${newEndpoint}:${newPort}`);
  console.log(`  ALLOWED_IPS=0.0.0.0/0,::/0`);
  console.log(`  PEER_EOF${c.reset}\n`);
}
