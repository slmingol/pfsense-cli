...
async function listBackends({ filter } = {}) {
  ...
  try {
    ...
    throw new Error(`Failed to list backends: ${error.response?.data?.message || error.message}`, { cause: error });
  }
}
...
async function addBackend({ name, serverName, serverAddress, serverPort, checkType = 'Basic', balance = 'roundrobin', ssl = false }) {
  ...
  try {
    ...
    throw new Error(`Failed to add backend: ${error.response?.data?.message || error.message}`, { cause: error });
  }
}
...
async function deleteBackend(name) {
  ...
  try {
    ...
    throw new Error(`Failed to delete backend: ${error.response?.data?.message || error.message}`, { cause: error });
  }
}
...
async function addFrontendRoute({ frontendName, aclName, hostname, backendName }) {
  ...
  try {
    ...
    throw new Error(`Failed to add frontend route: ${error.response?.data?.message || error.message}`, { cause: error });
  }
}
...
async function deleteFrontendRoute({ frontendName, aclName }) {
  ...
  try {
    ...
    throw new Error(`Failed to delete frontend route: ${error.response?.data?.message || error.message}`, { cause: error });
  }
}
...
async function resolveHostnameForIp(ip) {
  ...
  } catch { /* DNS lookup failure expected; caller handles null return */ }
  return null;
}
  ...
  for (const backend of backends) {
    for (const server of (backend.servers || [])) {
      ...
      try {
        ...
        ip = result.address;
      } catch { /* DNS lookup failure expected; caller handles null ip */ }
...
function restartHaproxy() {
  ...
  console.log(`Use haproxy:apply to regenerate the config and bring the daemon up,`);
  console.log(`or use the pfSense web GUI: Services > HAProxy > Apply.\n`);
  console.log(`${c.gray}Note: haproxy:apply does a reload, not a full restart. To clear stale _0/_1`);
  console.log(`DOWN entries you need pfSense GUI > Services > HAProxy > Restart.${c.reset}\n`);
}
...
async function inspectBackend({ name }) {
  const client = getPfSenseClient();
  const response = await client.get('/api/v2/services/haproxy/backends');
  // Strip pfSense frontend ACL suffixes (_ipvANY, _ipv4, _ipv6) before matching
  const normalizedName = name.replace(/_ipv(ANY|4|6)$/, '');
  const backends = (response.data.data || []).filter(b => b.name === normalizedName);
...
/**
 * Audit HAProxy backends for server address risk.
 * Categories:
 *   - static IP   ✓  safe, no DNS dependency
 *   - alias/svc   ~  service-specific hostname; functional but can go DOWN on reload
 *   - shared/host ⚠  primary DNS hostname with multiple aliases; highest risk
 */
async function auditBackends() {
  const client = getPfSenseClient();
  ...
  const backends = backendsRes.data.data || [];
  const dnsHosts = dnsRes.data.data || [];

  // primary FQDN → alias count
  ...
  for (const backend of backends) {
    for (const server of (backend.servers || [])) {
      ...
      if (IP_RE.test(addr)) {
        safe.push({ backend, server });
      } else if (hostAliasCount[addr] !== undefined && hostAliasCount[addr] >= 3) {
        shared.push({ backend, server, aliasCount: hostAliasCount[addr] });
      } else {
        named.push({ backend, server });
      }
    }
  }

  console.log(`\n${c.bold}HAProxy Backend Audit${c.reset}`);
  console.log(c.gray + '─'.repeat(72) + c.reset);
  console.log(`\n  ${c.green}${safe.length}${c.reset} static IP${safe.length !== 1 ? 's' : ''}  (safe)`);
  console.log(`  ${c.blue}${named.length}${c.reset} service hostname${named.length !== 1 ? 's' : ''}  (reload risk)`);
  console.log(`  ${c.yellow}${shared.length}${c.reset} shared/catch-all hostname${shared.length !== 1 ? 's' : ''}  (HIGH RISK)`);
  console.log(c.gray + '─'.repeat(72) + c.reset);

  if (shared.length > 0) {
    console.log(`\n${c.bold}${c.yellow}SHARED / CATCH-ALL (${shared.length})${c.reset}  ${c.gray}← convert with: haproxy:use-ip --apply${c.reset}`);
    for (const { backend, server, aliasCount } of shared) {
      console.log(`  ${c.yellow}⚠${c.reset}  ${c.cyan}${backend.name}${c.reset}  ${c.gray}(${server.name})${c.reset}`);
      console.log(`     ${c.yellow}${server.address}:${server.port}${c.reset}  ${c.gray}primary host with ${aliasCount} aliases${c.reset}`);
    }
  }

  if (named.length > 0) {
    console.log(`\n${c.bold}SERVICE HOSTNAME (${named.length})${c.reset}  ${c.gray}← can go DOWN on HAProxy reload${c.reset}`);
    for (const { backend, server } of named) {
      console.log(`  ${c.blue}~${c.reset}  ${c.cyan}${backend.name}${c.reset}  ${c.gray}(${server.name})${c.reset}`);
      console.log(`     ${server.address}:${server.port}`);
    }
  }

  if (safe.length > 0) {
    console.log(`\n${c.bold}STATIC IP (${safe.length})${c.reset}`);
    for (const { backend, server } of safe) {
      console.log(`  ${c.green}✓${c.reset}  ${c.cyan}${backend.name}${c.reset}  ${server.address}:${server.port}`);
    }
  }

  console.log('');
  if (shared.length > 0) {
    console.log(`${c.yellow}Run haproxy:use-ip --apply to convert all hostname backends to static IPs.${c.reset}`);
  } else if (named.length > 0) {
    console.log(`${c.gray}Run haproxy:use-ip --apply to eliminate reload risk.${c.reset}`);
  } else {
    console.log(`${c.green}All backends use static IPs.${c.reset}`);
  }
  console.log('');
}
...
async function listFrontends({ filter } = {}) {
  const client = getPfSenseClient();
  const response = await client.get('/api/v2/services/haproxy/frontends');
  let frontends = response.data.data || [];

  if (filter) {
    frontends = frontends.filter(f => f.name.toLowerCase().includes(filter.toLowerCase()));
  }

  console.log(`\n${c.bold}HAProxy Frontends (${frontends.length}):${c.reset}`);
  console.log(c.gray + '═'.repeat(80) + c.reset);
  console.log('');

  for (const [i, f] of frontends.entries()) {
    const mode    = f.mode || 'http';
    const bind    = f.bind || '(none)';
    const cert    = f.sslcertref ? `${c.yellow}[SSL: ${f.sslcertref}]${c.reset}` : '';
    const acls    = (f.ha_acls || []).length;
    const actions = (f.a_actionitems || []).length;

    console.log(`${c.gray}${i + 1}.${c.reset} ${c.bold}${c.cyan}${f.name}${c.reset}  ${c.gray}id=${f.id}${c.reset}`);
    console.log(`   ${c.gray}Bind:${c.reset}    ${bind}`);
    console.log(`   ${c.gray}Mode:${c.reset}    ${mode}`);
    if (cert) console.log(`   ${c.gray}Cert:${c.reset}    ${cert}`);
    console.log(`   ${c.gray}ACLs:${c.reset}    ${acls}  ${c.gray}Actions:${c.reset} ${actions}`);
    console.log('');
  }

  console.log(c.gray + '═'.repeat(80) + c.reset);
  console.log(`${c.gray}Total: ${frontends.length} frontends${c.reset}\n`);
}

async function addFrontend({ name, bind, mode = 'http', certName = null }) {
  const client = getPfSenseClient();

  // Check exists
  const listResp = await client.get('/api/v2/services/haproxy/frontends');
  const existing = (listResp.data.data || []).find(f => f.name === name);
  if (existing) {
    console.log(`  ${c.blue}ℹ${c.reset} Frontend already exists: ${c.cyan}${name}${c.reset}`);
    return;
  }

  let sslcertref = null;
  if (certName) {
    const certsResp = await client.get('/api/v2/system/certificates', { params: { limit: 0 } });
    const cert = (certsResp.data.data || []).find(c => c.descr === certName || c.refid === certName);
    if (!cert) throw new Error(`Certificate not found: ${certName}`);
    sslcertref = cert.refid;
  }

  const payload = { name, bind, mode };
  if (sslcertref) {
    payload.sslcertref = sslcertref;
    payload.ssl        = true;
  }

  const resp = await client.post('/api/v2/services/haproxy/frontend', payload);
  if (resp.data.code !== 200) throw new Error(resp.data.message || 'Failed to create frontend');

  console.log(`  ${c.green}✓${c.reset} Created frontend: ${c.cyan}${name}${c.reset} ${c.gray}(${bind}, ${mode})${c.reset}`);
  if (certName) console.log(`  ${c.green}✓${c.reset} SSL cert: ${c.yellow}${certName}${c.reset}`);
  await applyChanges(client);
}
...
async function deleteFrontend(name) {
  ...
  const listResp = await client.get('/api/v2/services/haproxy/frontends');
  const frontend = (listResp.data.data || []).find(f => f.name === name);
  if (!frontend) throw new Error(`Frontend not found: ${name}`);

  const resp = await client.delete('/api/v2/services/haproxy/frontend', { data: { id: frontend.id } });
  if (resp.data.code !== 200) throw new Error(resp.data.message || 'Failed to delete frontend');

  console.log(`  ${c.green}✓${c.reset} Deleted frontend: ${c.cyan}${name}${c.reset}`);
  await applyChanges(client);
}

async function assignFrontendCert({ name, certName }) {
  const client = getPfSenseClient();
  ...
  const certsResp = await client.get('/api/v2/system/certificates', { params: { limit: 0 } });
  const cert = (certsResp.data.data || []).find(ct => ct.descr === certName || ct.refid === certName);
  if (!cert) throw new Error(`Certificate not found: ${certName}`);

  const resp = await client.patch('/api/v2/services/haproxy/frontend', {
    id:         frontend.id,
    sslcertref: cert.refid,
    ssl:        true,
  });
  if (resp.data.code !== 200) throw new Error(resp.data.message || 'Failed to update frontend cert');

  console.log(`  ${c.green}✓${c.reset} Frontend ${c.cyan}${name}${c.reset} cert → ${c.yellow}${certName}${c.reset} ${c.gray}(${cert.refid})${c.reset}`);
  await applyChanges(client);
}

/**
 * Set timeout values on a HAProxy backend.
 * connection_timeout and server_timeout are top-level backend fields (milliseconds).
 * tunnel_timeout goes into advanced_backend as a raw HAProxy directive (ms).
 */
async function setBackendTimeouts({ name, connectTimeout, serverTimeout, tunnelTimeout, apply = true }) {
  const client = getPfSenseClient();
  const response = await client.get('/api/v2/services/haproxy/backends');
  const backend = (response.data.data || []).find(b => b.name === name);
  if (!backend) throw new Error(`Backend not found: ${name}`);

  const patch = { id: backend.id };

  if (connectTimeout !== undefined) patch.connection_timeout = parseInt(connectTimeout, 10);
  if (serverTimeout  !== undefined) patch.server_timeout     = parseInt(serverTimeout, 10);

  if (tunnelTimeout !== undefined) {
    // Merge into existing advanced_backend, replacing any prior timeout tunnel line
    const existing = (backend.advanced_backend || '').split('\n').filter(l => !l.trim().startsWith('timeout tunnel'));
    existing.push(`timeout tunnel ${tunnelTimeout}ms`);
    patch.advanced_backend = existing.filter(Boolean).join('\n');
  }

  if (Object.keys(patch).length === 1) {
    console.log(`  ${c.yellow}No timeout options specified.${c.reset}`);
    return;
  }

  const resp = await client.patch('/api/v2/services/haproxy/backend', patch);
  if (resp.data.code !== 200) throw new Error(resp.data.message || 'Patch failed');

  console.log(`\n  ${c.green}✓${c.reset} ${c.cyan}${name}${c.reset} timeouts updated:`);
  if (connectTimeout !== undefined) console.log(`    connection_timeout = ${connectTimeout}ms`);
  if (serverTimeout  !== undefined) console.log(`    server_timeout     = ${serverTimeout}ms`);
  if (tunnelTimeout  !== undefined) console.log(`    timeout tunnel     = ${tunnelTimeout}ms  ${c.gray}(advanced_backend)${c.reset}`);

  if (apply) await applyChanges(client);
}
