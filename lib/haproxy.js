...
async function restartHaproxy() {
  const client = getPfSenseClient();
  console.log(`${c.gray}Stopping HAProxy...${c.reset}`);
  try {
    await client.post('/api/v2/services/haproxy/stop');
  } catch (err) {
    // ignore -- daemon may already be stopping or endpoint may vary
    console.log(`  ${c.yellow}stop returned: ${err.response?.data?.message || err.message}${c.reset}`);
  }
  console.log(`${c.gray}Starting HAProxy...${c.reset}`);
  try {
    const response = await client.post('/api/v2/services/haproxy/start');
    if (response.data.code === 200) {
      console.log(`  ${c.green}✓${c.reset} HAProxy started`);
    } else {
      throw new Error(response.data.message || 'Unexpected response from start');
    }
  } catch (error) {
    throw new Error(`Failed to start HAProxy: ${error.response?.data?.message || error.message}`);
  }
}
