...
async function restartHaproxy() {
  const client = getPfSenseClient();
  console.log(`${c.gray}Restarting HAProxy...${c.reset}`);
  try {
    const response = await client.post('/api/v2/status/service', { name: 'haproxy', action: 'restart' });
    if (response.data.code === 200) {
      console.log(`  ${c.green}✓${c.reset} HAProxy restarted`);
    } else {
      throw new Error(response.data.message || 'Unexpected response');
    }
  } catch (error) {
    throw new Error(`Failed to restart HAProxy: ${error.response?.data?.message || error.message}`);
  }
}
