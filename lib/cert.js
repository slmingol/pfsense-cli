...
function fmtExpiry(daysLeft) {
  if (daysLeft === null || daysLeft === undefined) return c.gray + '?' + c.reset;
...
async function listCerts({ filter, expiringDays } = {}) {
  ...
  if (expiringDays !== null && expiringDays !== undefined) {
    certs = certs.filter(ct => ct.valid_days_left !== null && ct.valid_days_left !== undefined && ct.valid_days_left <= expiringDays);
  }

  if (certs.length === 0) {
    console.log(filter || (expiringDays !== null && expiringDays !== undefined)
...
async function checkCerts({ expiringDays = 30 } = {}) {
  ...
  const expiring = certs.filter(ct => ct.valid_days_left !== null && ct.valid_days_left !== undefined && ct.valid_days_left <= expiringDays);
